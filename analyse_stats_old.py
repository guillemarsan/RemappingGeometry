...
def read_spikebins(path,f):
    spikebins = read_matrix(path,f,'pfs')
    return spikebins
        ...
        for dim_idx, dim  in enumerate(dim_vect):
            for dir_idx, dir in enumerate(dir_vect):
                for loadid_idx, loadid in enumerate(loadid_vect):
                    ...
                    meanfr = np.zeros((all_ratemaps.shape[0],b))
                    for j in np.arange(b):
                        ...
                        meanfr[:,j] = prob[j]*all_ratemaps[:,j]

                    meanfr = np.sum(meanfr, axis=1)
                    for j in np.arange(b):
                    ...
                    if np.max(dict['xaxis']) <  np.max(results_spatialinfo):
                        xaxis = [(a+info_bins[i+1])/2.0 for i,a in enumerate(info_bins[0:-1])]
                        xaxis[0] = 0
                        dict['xaxis'] = xaxis
...
if __name__ == "__main__":
    ...
    parser.add_argument("--compute", type=str, default='spatialoverlap',
                        help = 'Which thing to analyse to make')
    parser.add_argument("--shuffle", action='store_true', default=False,
                        help="Shuffle the data in some way")
    parser.add_argument("--tag", type=str, default='condB',
