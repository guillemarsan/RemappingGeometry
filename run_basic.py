...
if __name__ == "__main__":
    ...
    model, D = get_model(inp,n,out,connectivity=args.model)
    ...
    results['error'] = np.linalg.norm(V - (model.F @ x - model.G @ y))
    ...
    if args.plot:
        ...
        if inp == 1 and out == 1:
            plot.plot_iofunc(x, y, model.F, model.G, model.T, basepath)

        
        if args.input == 'cst' and (out == 2 or out==3):
            print('Generating bounding box plot...')
            if out == 2:
                plot.plot_1dbbox(x[:,-1], y, model.F, model.G, model.T, basepath, plotx=(args.model == 'randae' or args.model == 'polyae'))
            else:
                plot.plot_2dbboxproj(x[:,-1], y, model.F, model.G, model.T, basepath, plotx=(args.model == 'randae' or args.model == 'polyae'))

        if inp == 1 or (args.input == 'circle' and inp == 2):
            ...
            plot.plot_1drfs(p, r, dt, basepath)
            plot.plot_1dspikebins(p, s, 16, basepath)
        
        if args.input == 'spiral' and inp == 3:
    ...
    if args.gif and out == 2:
        ...
        plot.plot_1danimbbox(x, y, model.F, model.G, model.T, basepath, plotx=(args.model == 'randae' or args.model == 'polyae'))