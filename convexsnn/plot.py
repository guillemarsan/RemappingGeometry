...
def plot_1dbbox(x, y, F, G, T, basepath, plotx = False, foranim=False):
    ...
    for i in range(n):
        ...
        if foranim and y2 == line_func(i,y1):
            plt.plot(lim, line, color=c[i], linewidth = 10)
        else:
            plt.plot(lim, line, color=c[i])
        if not foranim:
            ...
            plt.imshow((np.sign(G[i,1])*py > np.sign(G[i,1])*line_func(i,px)).astype(int), extent=(px.min(),px.max(),py.min(),py.max()),
                cmap=colors.ListedColormap([c[i], 'white']), aspect='auto', origin="lower", alpha=0.3)
...
def plot_2dbboxproj(x, y, F, G, T, basepath, plotx = False):
    ...
    for i in range(n):
        plt.imshow((maxidx == i).astype(int), extent=(lmin,lmax,lmin,lmax),
                cmap=colors.ListedColormap([c[i]]), aspect='auto', origin="lower", alpha=(maxidx==i).astype(float)*0.75)
    plt.xlabel('y1')
    plt.ylabel('y2')
    plt.xlim(lmin,lmax)
    plt.ylim(lmin,lmax)
...
def plot_2dbbox(x, y, F, G, T, basepath, plotx = False, foranim=False):
    ...
    for i in range(n):
        ...
        if foranim and y3 == plane_func(i,y1,y2):
            ax3.plot_surface(X=lim1, Y=lim2, Z=planes[:,:,i], color=c[i])
        else:
            ax3.plot_surface(X=lim1, Y=lim2, Z=planes[:,:,i], color=c[i], alpha=0.3)
...
def plot_neuroscience(x, y, V, s, t, basepath):
    ...
    for i in range(V.shape[0]):
        ax2.plot(t,V[i,:], label='V' + str(i), color=c[i])
        ax2.vlines(t[s[i,:] == 1], maxV + (i+1)*off + i*l, maxV + (i+1)*off + (i+1)*l, color=c[i])
    ax2.legend()
    ax2.set_ylabel('V(mV)')
    ax2.set_xlabel('t(ms)')
...
def plot_1drfs(p, r, dt, basepath, pad=0):
    ...
    for i in range(r.shape[0]):
        ...
        plt.plot(p, rf, label='r' + str(i), color=c[i])
    plt.legend()
    plt.ylabel('r')
    plt.xlabel('p')
